...
# Python 2.7 compat: open() nao aceita encoding/errors, entao wrappeamos.
...
if _py_sys.version_info[0] >= 3 and _py_sys.version_info[1] >= 6:
...
# Python 2.7 compat: urlopen() nao aceita context= nem with_statement
...
def _touch_session():
    try:
        with _open(SESSION_PATH, "w", encoding="utf-8") as f:
...
try:
    with _open(CONFIG_PATH, "r", encoding="utf-8") as f:
        CONFIG = json.load(f)
except Exception:
    CONFIG = {}
...
def _load_cache():
    ...
    try:
        if os.path.exists(CACHE_PATH):
            with _open(CACHE_PATH, "r", encoding="utf-8") as f:
                _cache = json.load(f)
            log.info("Cache carregado: {} frases".format(len(_cache)))
    except Exception as e:
        log.error("Falha carregar cache: {}".format(e))
        _cache = {}
    try:
        if os.path.exists(WORDS_PATH):
            with _open(WORDS_PATH, "r", encoding="utf-8") as f:
                _words = json.load(f)
            log.info("Palavras carregadas: {}".format(len(_words)))
    except Exception:
        _words = {}
    ...
    for path in (CACHE_PATH, WORDS_PATH):
        ...
        if parent and not os.path.isdir(parent):
            try:
                ...
                pass
    try:
        with _open(CACHE_PATH, "w", encoding="utf-8") as f:
            json.dump(cache_snapshot, f, ensure_ascii=False, separators=(",", ":"))
            f.flush()
    except Exception as e:
        log.error("Falha salvar cache: {}".format(e))
        with _cache_lock:
            _cache_dirty = True
    try:
        with _open(WORDS_PATH, "w", encoding="utf-8") as f:
            json.dump(words_snapshot, f, ensure_ascii=False, separators=(",", ":"))
            f.flush()
    except Exception as e:
        log.error("Falha salvar palavras: {}".format(e))
...
# Python 2.7 compat: str vs unicode
try:
    _str_type = (str, unicode)
except NameError:
    _str_type = str

# Python 2.7 compat: json.JSONDecodeError nao existe
try:
...
def _translate_google_v1(texts):
    ...
    resp = _urlopen(req, timeout=15, context=ctx)
    try:
...
def _translate_google_v2(texts):
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            for item in items:
                if isinstance(item, list) and len(item) >= 2:
                    resp_str = item[1]
                    if resp_str and isinstance(resp_str, _str_type):
                        try:
        ...
        log.error("V2 JSON error: {}".format(e))
        log.hook("V2 raw[:300]: {}".format(repr(raw[:300])))
    while len(out) < len(texts):
        out.append(texts[len(out)])
    return out
...
def _translate_local(texts):
    ...
    def request_pair(values, source, target):
        ...
        req = Request(endpoint, data=json.dumps(payload).encode("utf-8"), headers={"Content-Type": "application/json"})
...
def translate_batch(texts, retries=2):
    ...
    log.hook("[BATCH_DEBUG] chamado com {} textos, PROVIDER={}".format(len(texts), PROVIDER))
    if not texts:
        return []
    order = []
    if PROVIDER == "local":
    ...
    log.hook("[BATCH] Ordem: {}".format(' -> '.join(n for n, _ in order)))
    for retry in range(retries + 1):
        if retry > 0:
            time.sleep(1.5)
            log.warn("Retry {}/{}...".format(retry, retries))
        for pname, pfunc in order:
            try:
                result = pfunc(texts)
                diff = sum(1 for t, r in zip(texts, result) if r and r != t)
                log.api("{}: {}/{} traduzidos".format(pname, diff, len(texts)))
                for t, r in zip(texts, result):
                    if r and r != t:
                        log.ok("TRAD: {} -> {}".format(repr(t[:80]), repr(r[:80])))
                    elif r == t and len(t) > 3:
                        log.hook("IDENT {}: {}".format(pname, repr(t[:60])))
                        if pname == "google_free":
                            log.hook("[V1 ECHO] enviado={} recebido={}".format(repr(t[:80]), repr(r[:80])))
                ...
                if e.code == 429:
                    log.warn(">>> RATE LIMIT: {}, tentando alternativo...".format(pname))
                else:
                    log.error("Erro HTTP {} em {}".format(e.code, pname))
            except Exception as e:
                log.error("Erro {}: {}".format(pname, e))
    ...
    while True:
        ...
        if PROVIDER == "local" and uat_lt is not None and not uat_lt.is_ready():
            if time.time() < uat_lt.deadline():
                ...
                time.sleep(1.0)
                continue
        log.hook("[WORKER] pegou {} da fila".format(len(batch)))
        n = len(batch)
        log.hook("[WORKER] Traduzindo lote de {} texto(s)...".format(n))
        ...
        try:
            ...
            log.error("[WORKER] lote erro: {}".format(e))
        ...
        for i, (orig, prot, tokens) in enumerate(batch):
            try:
                ...
                if not isinstance(trans, _str_type):
                    trans = _str_type(trans)
                trans = _restore_renpy(trans, tokens)
                _extract_words(orig, trans)
                translated[i] = trans
                log.hook("[WORKER] item {}: res={}".format(i, repr(trans[:60])))
            except Exception as e:
                log.error("[WORKER] item {} erro: {}".format(i, e))
                translated[i] = prot
        try:
            with _cache_lock:
            ...
            log.hook("[WORKER] Lote concluido: {}/{} traduzidos".format(acertos, n))
            if acertos:
                _request_ui_refresh()
            ...
            log.error("Worker erro: {}".format(e))
            with _queue_lock:
                for b in batch:
...
def _translate_text(text, blocking=False):
    ...
    if not isinstance(text, _str_type) or not text.strip():
        return text
    try:
        with _cache_lock:
        ...
        if blocking:
            ...
            try:
                ...
                log.warn("Block traduccao falhou: {}".format(e))
            return text
    ...
    try:
        ...
        if target is not None:
            try:
                ...
                log.ok("TRADUZ (apos aparecer): {}".format(repr(translated[:80])))
                return
            except Exception as e:
                log.warn("set_text no widget falhou: {}".format(e))
        ...
        try:
            ...
            log.ok("TRADUZ (apos aparecer, fallback): {}".format(repr(translated[:80])))
        except Exception as e:
            log.warn("Falha re-render dialogo: {}".format(e))
    except Exception as e:
        log.warn("Falha re-render dialogo: {}".format(e))
...
def _perform_ui_refresh():
    """Reconstroi a interacao para menus ja visiveis relerem o cache."""
    try:
        _apply_waiting_ui_widgets()
        import renpy
        renpy.restart_interaction()
        log.hook("[UI] interacao atualizada apos traducao")
    except Exception as e:
        log.warn("Falha atualizar UI traduzida: {}".format(e))
...
def _request_ui_refresh():
    """Agrupa conclusoes proximas em um unico restart_interaction."""
...
def _ensure_save_wrap():
    ...
    try:
        ...
        log.warn("save-wrap adiado: {}".format(e))
    try:
        import renpy
        def _cb():
            if _install_save_wrap():
        ...
        log.warn("Falha agendar save-wrap: {}".format(e))

def install_hook():
    try:
        import renpy
    except Exception as e:
        log.error("renpy nao disponivel: {}".format(e))
        return

    log.info("=== Instalando hooks (Fase 2) ===")
    log.info("Provider: {} | {} -> {}".format(PROVIDER, SOURCE, TARGET))
    log.info("Cache: {} ({} entradas)".format(CACHE_PATH, len(_cache)))
    log.info("Tradutores disponiveis: google_v1, google_v2, deepl_free")
    log.info("Status: {}".format('CACHE PRONTO' if _cache else 'SEM CACHE (primeira execucao)'))
    ...
    try:
        ...
        log.warn("Falha desligar say_menu_text_filter: {}".format(e))
    ...
    try:
        ...
        def patched_say_execute(self):
            log.hook("[SAY_EXECUTE] who={}, what={}".format(self.who, repr(self.what)[:120]))
        ...
        log.warn("Falha hook Say.execute: {}".format(e))
        ...
        try:
            ...
            def _make_patched(fname, orig):
                def patched(*args, **kwargs):
                    if len(args) >= 2 and isinstance(args[1], _str_type):
                        log.hook("[{}] what={}".format(fname, repr(args[1])[:120]))
                    elif 'what' in kwargs and isinstance(kwargs['what'], _str_type):
                        log.hook("[{}] what={}".format(fname, repr(kwargs['what'])[:120]))
            ...
            if _func == "ADVCharacter.__call__":
                ...
                def _patched_do_display(self, who, what, *args, **kwargs):
                    ...
                    if isinstance(what, _str_type) and len(what) > 3:
                        with _dialogue_lock:
                            _current_dialogue["what"] = what
                            _current_dialogue["who"] = who
                        cached = _translate_text(what, blocking=True)
                        if cached != what:
                            log.ok("DIALOGO: {}".format(repr(what[:80])))
                            log.ok("TRADUZ: {}".format(repr(cached[:80])))
                            what = cached
                        else:
                            if what not in _logged_hooks:
                                _logged_hooks.add(what)
                                log.hook("DIALOGO (original): {}".format(repr(what[:80])))
            ...
            log.hook("Hook OK: {} (log-only)".format(_func))
        except Exception as e:
            log.warn("Falha hook {}: {}".format(_func, e))
    ...
    try:
        ...
        if Text:
            ...
            def _patched_set_text(self, text, scope=None, substitute=False, update=True):
                orig_text = text
                if isinstance(text, _str_type) and len(text) > 3:
            ...
            def _patched_text_init(self, *args, **kwargs):
                if args and isinstance(args[0], _str_type) and len(args[0]) > 3:
                    with _cache_lock:
                    ...
                    if not is_trans:
                        ...
                        if cached != args[0]:
                            args = (cached,) + args[1:]
                elif 'text' in kwargs and isinstance(kwargs['text'], _str_type) and len(kwargs['text']) > 3:
                    with _cache_lock:
            ...
            try:
                ...
                log.warn("Falha hook Text.__init__: {}".format(e))
    except Exception as e:
        log.warn("Falha hooks Text: {}".format(e))
    ...
    try:
        ...
        def _patched_show_screen(name, *args, **kwargs):
            if name == "say" and 'what' in kwargs and isinstance(kwargs['what'], _str_type) and len(kwargs['what']) > 3:
                w = kwargs['what']
                # Usa cache se ja traduzido (do_display ja populou); senao enfileira
                cached = _translate_text(w, blocking=False)
                if cached != w:
                    kwargs['what'] = cached
            log.hook("[SHOW_SCREEN] name={}, who={}, what={}".format(name, repr(kwargs.get('who',''))[:60], repr(kwargs.get('what',''))[:120]))
        ...
        log.warn("Falha hook show_screen: {}".format(e))
    ...
    try:
        ...
        def patched_disp_init(self, *args, **kwargs):
            ...
            log_once("DISP_{}".format(cls_name), "Displayable criado: {}".format(cls_name))
        ...
        log.warn("Falha hook Displayable: {}".format(e))
                            ...
                            if not is_trans:
                                ...
                                if cached != args[0]:
                                    ...
                                    if args[0] not in _logged_hooks:
                                        ...
                                        log.ok("UI_{}: {}".format(fname, repr(args[0][:60])))
                        elif 'text' in kwargs and isinstance(kwargs['text'], _str_type) and len(kwargs['text']) > 3:
                            with _cache_lock:
                                is_trans = kwargs['text'] in _cache and _cache[kwargs['text']] == kwargs['text']
                            if not is_trans:
                                ...
                                if cached != kwargs['text']:
                                    ...
                                    if kwargs['text'] not in _logged_hooks:
                                        ...
                                        log.ok("UI_{}: {}".format(fname, repr(kwargs['text'][:60])))
                        return orig(*args, **kwargs)
                    return _patched
                _p = _make_ui_patched(_uiname.upper(), _orig_ui)
                setattr(renpy.ui, _uiname, _p)
                _record_patch(renpy.ui, _uiname, _p, _orig_ui)
                log.hook("Hook OK: renpy.ui.{}".format(_uiname))
    except Exception as e:
        log.warn("Falha hook ui: {}".format(e))
    ...
    try:
        ...
        def patched_sayexports(who, what, *args, **kwargs):
            log.hook("[SAYEXPORTS] what={}".format(repr(what)[:120]))
        ...
        log.warn("Falha hook sayexports: {}".format(e))
    ...
    def _heartbeat():
        ...
        while True:
            ...
            count += 1
            _touch_session()
            log.hook("Heartbeat {}: hook ativo".format(count))
    ...
    if not _worker_thread_started:
        ...
        wt = threading.Thread(target=_worker_loop)
        wt.daemon = True
        wt.start()
    ...
    try:
        ...
        log.warn("Falha instalar refresh periodico de UI: {}".format(e))
    ...
    if PROVIDER == "local" and uat_lt is not None:
        try:
            ...
            log.warn("Falha LT ensure (fase2): {}".format(e))
...
def phase1():
    ...
    if SHOW_CONSOLE:
        try:
            ...
            sys.stdout = _open("CONOUT$", "w", encoding="utf-8", buffering=1)
            sys.stderr = _open("CONOUT$", "w", encoding="utf-8", buffering=1)
            try:
                log.reopen_console()
            except Exception:
                pass
    ...
    if PROVIDER == "local" and uat_lt is not None:
        try:
            ...
            log.warn("Falha iniciar LT manager: {}".format(e))
