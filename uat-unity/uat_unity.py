...
def translate_text(text, from_code, to_code):
    '''Traduz usando Argos diretamente. from='auto'/'' -> source da config.'''
    if not text:
        return ''
    cfg = load_config()
    if from_code in ('auto', '', None):
        from_code = _argos_code(cfg.get('source_language', 'en'))
    else:
        from_code = _argos_code(from_code)
    to_code = _argos_code(to_code)
    try:
        with _translate_lock:
            if not _ensure_pair_loaded(from_code, to_code):
                return text
            _refresh_argos()
            langs = {l.code: l for l in _atranslate.get_installed_languages()}
            src_lang = langs.get(from_code)
            tgt_lang = langs.get(to_code)
...
def configure_languages(source_code=None, target_code=None):
    ...
    if source_code:
        ...
        if src is None:
            return False
    targets = sorted({t for s, t in available if s == src}, key=lambda c: names.get(c, c).lower())
    if not targets:
    ...
    if target_code:
        ...
        if tgt not in targets:
            print('[ERRO] Par direto indisponivel: {} -> {}'.format(src, target_code))
            return False
    else:
    ...
    print('\n  Par: {} -> {}'.format(src, tgt))
    if (src, tgt) not in installed_models():
        ans = ('s' if scripted else
               _prompt('\nModelo {} -> {} nao instalado. Baixar agora? [S/n]: '.format(src, tgt), 's'))
        if ans.lower() in ('s', '', 'y', 'sim', 'yes'):
            ...
            print('[AVISO] Download cancelado; a configuracao anterior foi mantida.')
            if not scripted:
                _prompt('\nENTER para voltar...')
            return False
    else:
        print('[OK] Modelo ja instalado.')
    if not validate_pair(src, tgt):
        print('[ERRO] O par nao ficou operacional; a configuracao anterior foi mantida.')
        if not scripted:
            _prompt('\nENTER para voltar...')
        return False
...
def main():
    ...
    if command in ('config', 'configure', 'idiomas', 'languages'):
        ...
        configure_languages(source, target)
        return
